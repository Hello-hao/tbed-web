  ...
  <Layout style="margin-top: 50px;margin-bottom: 50px;">
    <Content :style="{margin: '15px 20px 0'}">
      <p style="position: fixed;right: 58px;z-index: 1;bottom: 68px;">
        <Button type="primary" shape="circle" icon="md-add-circle"
                style="z-index: 1;margin-right: 8px;box-shadow: rgba(0, 0, 0, 0.4) 0px 2px 6px 0px;" @click="showAdd">添
          加
        </Button>
      </p>
    ...
    <Modal v-model="isAdd" :footer-hide="true">
      ...
      <Form @submit.native.prevent :label-width="70">
        <FormItem label="群组名称">
          <Input :disabled="(formItem.id==1 && urlType=='edit')" v-model="formItem.groupname" placeholder="群组名称"
                 maxlength="20" size="large"></Input>
        </FormItem>
        <FormItem label="存储源">
        ...
        <FormItem label="用户组">
          <Select :disabled="(formItem.id==1 && urlType=='edit')" v-model="formItem.usertype" placeholder="用户组"
                  size="large">
