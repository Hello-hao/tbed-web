...
<template>
  ...
  <Layout style="margin-bottom: 50px;">
    ...
    <Content :style="{margin: '15px 20px 0', minHeight: '500px'}">
      <Row :gutter="16" v-if="this.$store.state.RoleLevel=='admin' "
           class="animate__animated animate__fadeIn animate__delay-0.6s">
        <Col :xs="24" :sm="12" :md="12" :lg="6">
          <Card :shadow="true" class="cardShadow">
            <p slot="title">图片总数
              <Tooltip content="昨日新增图像" theme="light" style="float: right;">
                <Badge :count="0" class-name="badge-alone"></Badge>
              </Tooltip>
            </p>
            <div class="cardstyle">
        ...
        <Col :xs="24" :sm="12" :md="12" :lg="6">
          <Card :shadow="true" class="cardShadow">
            <p slot="title">用户总数
              <Tooltip content="昨日新增用户" theme="light" style="float: right;">
                <Badge :count="0" class-name="badge-alone"></Badge>
              </Tooltip>
            </p>
            <div class="cardstyle">
        ...
        <Col :xs="24" :sm="12" :md="12" :lg="6">
          <Card :shadow="true" class="cardShadow">
            <p slot="title">累计违规
              <Tooltip :content="ViolationSwitch>0?ViolationSwitch==1?'百度审核:开':'ModerateContent:开':'图像审核:关'"
                       theme="light" style="float: right;">
                <Icon type="md-radio-button-off" size="20" color="#499c54" v-show="ViolationSwitch>0"/>
                <Icon type="md-power" size="20" color="#c75450" v-show="ViolationSwitch==0"/>
              </Tooltip>
            </p>
            <div class="cardstyle">
        ...
        <Col :xs="24" :sm="12" :md="12" :lg="6">
          <Card :shadow="true" class="cardShadow">
            <p slot="title">访客容量
              <Tooltip :content="VisitorUpload==0?'访客上传:关':'访客总容量'" theme="light"
                       style="position: absolute; right: 20px;">
                <Badge :text="VisitorMemory" class-name="badge-alone"></Badge>
              </Tooltip>
            </p>
            <div class="cardstyle" style="text-align: center;">
              <i-circle :percent="VisitorProportion" :size="80" stroke-color="#ff9c6e" :stroke-width="9"
                        :trail-width="9">
      ...
      <Row class="animate__animated animate__fadeIn animate__delay-0.8s">
        <Col flex="1" style="padding: 8px;">
          <Card class="cardShadow" style="margin-top: 10px;padding-left: 8px; padding-right: 8px;">
            <div class="mydiv">
              <p>
                <Icon type="md-photos" style="color: #69c0ff;" class="myico"/>
              </p>
        ...
        <Col flex="1" style="padding: 8px;">
          <Card class="cardShadow" style="margin-top: 10px;padding-left: 8px; padding-right: 8px;">
            <div class="mydiv">
              <p>
                <Icon type="ios-color-palette" style="color: #ff9c6e;" class="myico"/>
              </p>
          ...
          <Card class="cardShadow" style="margin-top: 10px; padding-left: 8px; padding-right: 8px;">
            <div class="mydiv" style="padding: 6px 15px;">
              <Tooltip content="我的容量" theme="light" placement="right-start">
                <i-circle :percent="myMemory" dashboard :size="70" stroke-color="#30c479" :stroke-width="9"
                          :trail-width="9">
                  <p style="font-size:11px">{{ myMemory }}%</p>
                  <p @click="ShowSpaceExpansion"
                     style="font-size: smaller;color: #32a0e9; cursor:pointer; line-height: 23px;font-weight: 300;"><u>扩容</u>
                  </p>
                </i-circle>
        ...
        <Col flex="1" style="padding: 8px;">
          <Card class="cardShadow" style="margin-top: 10px;">
            <div class="mydiv" style="cursor:pointer;" @click="showToken">
              <p>
                <Icon type="md-key" style="color: #5cdbd3;" class="myico"/>
              </p>
              ...
              <Col :xs="24" :sm="12" :md="16" :lg="18">
                <div id="fdiv" style="width: 100%;">
                  <div style="position: absolute;z-index: 1;">
                    <p slot="title">
                      <Dropdown style="margin-left:15px;border-radius: 50%;" trigger="click"
                                v-if="this.$store.state.RoleLevel=='admin' && ok=='true'">
                        <Button size="small">
                          {{ this.viewName }}&nbsp;&nbsp;
                          <Icon type="ios-arrow-down"></Icon>
                        </Button>
                        <DropdownMenu slot="list">
                          <DropdownItem @click.native="viewType(1)">
                            <Icon type="ios-podium-outline"/>
                            个人
                          </DropdownItem>
                          <DropdownItem @click.native="viewType(2)">
                            <Icon type="ios-podium"/>
                            全站
                          </DropdownItem>
                        </DropdownMenu>
                      </Dropdown>
                      <Select v-model="yyyy" placeholder="年份" size="small" style="width: 80px;margin-left:15px;"
                              @on-change="viewTime">
                      ...
                      <div style="height: 290px;width: 100%;padding: 8px;overflow: auto">
                        <List>
                          <ListItem v-for="(user,index) in Recently.RecentlyUser" :key="index">
                            <div v-if="index==0">
                              <ListItemMeta :avatar="top1" :title="user.username"
                                            :description="'累计上传 '+user.counts+' 张图像'"/>
                            </div>
                            <div v-else-if="index==1">
                              <ListItemMeta :avatar="top2" :title="user.username"
                                            :description="'累计上传 '+user.counts+' 张图像'"/>
                            </div>
                            <div v-else-if="index==2">
                              <ListItemMeta :avatar="top3" :title="user.username"
                                            :description="'累计上传 '+user.counts+' 张图像'"/>
                            </div>
                            <div v-else>
                              <ListItemMeta :avatar="usertop" :title="user.username"
                                            :description="'累计上传 '+user.counts+' 张图像'"/>
                            </div>
                          </ListItem>
                        </List>
    ...
    <Modal
      ...
      <p style="text-align: center;"><Input v-model="SpaceCode" size="large" prefix="md-pint"
                                            placeholder="请输入你的扩容码" style="width: 88%"/></p>
    </Modal>
