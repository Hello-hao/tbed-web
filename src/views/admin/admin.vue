  ...
  <div class="layout">
    <Layout :style="{minHeight: '100vh'}">
      <Layout>
        ...
        <div style="width: 100%;height: 100%;background-color: rgba(51,51,51,0.36);position: fixed;z-index: 9;" @click="showMenu" v-show="isCollapsed"></div>
        ...
        <div class="layout-nav" v-show="true">
          <Menu mode="horizontal" theme="light" active-name="1">
            <MenuItem name="men01">
              <img @click="goHome" :src="$store.state.metaInfo.logo==null?hellohaologo:$store.state.metaInfo.logo" style="width: 135px;vertical-align: middle;"  />
            </MenuItem>
            ...
            <template v-if="screenWidth>1024">
              ...
              <Submenu name="men5">
                ...
                <MenuItem name="usersetting" @click.native="adminmenuItem('/usersetting','我的设置')">密码修改</MenuItem>
                <MenuItem name="api" v-if="$store.state.metaInfo.api==1" @click.native="adminmenuItem('/api','我的设置')">API文档</MenuItem>
              </Submenu>
              ...
              <!--                  </MenuGroup>-->
              <!--                </Submenu>-->
              <Icon type="md-exit" class="exit-style" title="退出登录" @click.native="exit" size="28" style="float: right; margin-right: 20px; line-height: 55px;cursor: pointer;" />
            </template>
            <template v-else>
              <Icon @click.native="showMenu" type="md-menu" size="28" style="float: right; margin-right: 20px; line-height: 55px; " />
            </template>
          </Menu>
        </div>

        <!--    移动端状态栏   -->
        <div v-show="isMenu!='none'" class="move-style animate__animated" :class="{'animate__fadeInDown' :isMenu=='show','animate__fadeOutUp' :isMenu=='hide'}">
          <p style="text-indent: 8px;" @click="adminmenuItem('/',null)"><Icon type="md-speedometer" size="16"/>&nbsp;&nbsp;&nbsp;&nbsp;<span style="font-weight: bold">仪表盘</span></p>
          <Collapse simple>
            <Panel name="1">
              <span style="font-weight: bold">个人中心</span>
              <List size="small" slot="content" :split="false">
                <ListItem style="text-indent: 34px;" @click.native="adminmenuItem('/photo','个人中心')">我的相册</ListItem>
                <ListItem style="text-indent: 34px;" @click.native="adminmenuItem('/gallery','个人中心')">画廊管理</ListItem>
              </List>
            </Panel>
            <Panel name="2">
              <span style="font-weight: bold">我的设置</span>
              <List size="small" slot="content" :split="false">
                <ListItem style="text-indent: 34px;" @click.native="adminmenuItem('/usersetting','我的设置')">密码修改</ListItem>
                <ListItem style="text-indent: 34px;" v-if="$store.state.metaInfo.api==1" @click.native="adminmenuItem('/api','我的设置')">API文档</ListItem>
              </List>
            </Panel>
            <Panel name="3" v-if="$store.state.RoleLevel=='admin'">
              <span style="font-weight: bold">用户管理</span>
              <List size="small" slot="content" :split="false">
                <ListItem style="text-indent: 34px;" @click.native="adminmenuItem('/user','个人中心')">用户列表</ListItem>
                <ListItem style="text-indent: 34px;" @click.native="adminmenuItem('/group','个人中心')">分发群组</ListItem>
                <ListItem style="text-indent: 34px;" @click.native="adminmenuItem('/capacity','个人中心')">扩容码</ListItem>
              </List>
            </Panel>
            <Panel name="4" v-if="$store.state.RoleLevel=='admin'">
              <span style="font-weight: bold">系统设置</span>
              <List size="small" slot="content" :split="false">
                <ListItem style="text-indent: 34px;" @click.native="adminmenuItem('/storage','系统设置')">存储设置</ListItem>
                <ListItem style="text-indent: 34px;" @click.native="adminmenuItem('/setting','系统设置')">站点设置</ListItem>
                <ListItem style="text-indent: 34px;" @click.native="adminmenuItem('/moresetting','系统设置')">更多设置</ListItem>
              </List>
            </Panel>
            <Panel name="5" v-if="$store.state.RoleLevel=='admin'">
              <span style="font-weight: bold">更 多</span>
              <List size="small" slot="content" :split="false">
                <ListItem style="text-indent: 34px;" @click.native="adminmenuItem('/about','关于程序')">关于程序</ListItem>
              </List>
            </Panel>

          </Collapse>
          <p style="text-indent: 8px;" @click="exit"><Icon type="md-exit" size="16"/>&nbsp;&nbsp;&nbsp;&nbsp;<span style="font-weight: bold">退 出</span></p>
        </div>
