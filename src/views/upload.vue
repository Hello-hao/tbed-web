  ...
  <div>
      ...
      <Row class="animate__animated animate__delay-0.8s animate__fadeInDown" style="overflow: hidden;">
        <Col flex="1" style="padding: 8px;">
          <Card class="cardShadow"
                ...
                style="background: #FFF;margin-top: 10px;padding-left: 8px; padding-right: 8px;cursor:pointer;"
        ...
        <Col flex="1" style="padding: 8px;">
          <Card class="cardShadow"
                ...
                @mouseover="mouseOver"
                @mouseleave="mouseLeave"
                style="margin-top: 10px;padding-left: 8px; padding-right: 8px;cursor:pointer;"
    ...
    <Card id="box" class="mycard upcard img_list_box animate__animated  animate__delay-0.8s animate__fadeInUp"
      ...
      <div class="img_list_arr" style="text-align:center;">
        <div class="demo-upload-list animate__animated animate__delay-1.5s  animate__fadeIn"
             v-for="(item,index) in uploadList" v-bind:key="item.uid">
          <template v-if="item.status === 'finished'">
            ...
            <div class="demo-upload-list-cover">
              <Icon type="md-checkmark-circle" style="margin: 0 5px;"
                    :style="{color:(selectIndexUid.indexOf(item.uid)>=0?'#43b984':'#c7c7c7')}"
                    @click.native="clickImg(item.response.data.imguid,item.uid)" title="选择"></Icon>
        ...
        <Upload
          ...
          <div style="width: 100%;height:118px;">
            <div
                style="width: 98px;height: 64px;margin: 0px auto;text-align: center;position: absolute;left: 0;right: 0;top: 30px;"
                v-show="!getUploadSwitch">
              <Icon type="ios-camera" size="36"></Icon>
              <p style="text-align: center;color: #515a6e;font-size: xx-small;">拖拽图像到此处</p>
            </div>
            <div style="height: 64px;margin: 0px auto;text-align: center;position: absolute;left: 0;right: 0;top: 30px;"
                 v-show="getUploadSwitch">
          ...
          <List :split="false">
            ...
            <ListItem>
              <Input v-model="imgLinkForMD" class="cobyOrderSn" data-clipboard-action="copy"
                     :data-clipboard-text="imgLinkForMD" @click.native="copy"><span
                  slot="prepend">Markdown</span></Input>
            </ListItem>
          </List>
        </Modal>
        ...
        <Modal v-model="isAlbum" :footer-hide="true" :title="albumData.title">
          <Form @submit.native.prevent :label-width="70" style="margin-top: 30px;">
            ...
            <FormItem label="密码">
              ...
              <Button style="position: absolute;right: 30px;" size="small" type="primary" shape="circle"
                      class="cobyOrderSn" data-clipboard-action="copy"
        ...
        <Modal v-model="urlUploadMsg" :footer-hide="true">
          ...
          <Form @submit.native.prevent>
            ...
            <FormItem>
              <Button :disabled="tempURLErr" type="primary" :loading="loading" icon="md-cloud-upload" long
                      shape="circle" @click="uploadForUrl">UPLOAD
              </Button>
            </FormItem>
          </Form>
        </Modal>
