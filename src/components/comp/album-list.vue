<template>
  <div style="height: 510px;">
    <div style="margin:0 auto;height: 500px;overflow-x: hidden;overflow-y: auto;">
      ...
      <Row>
        <div style=" height: 40px; width: 95%;margin: auto; text-align: center; line-height: 50px;margin-top: 10px;" v-if="ischange">
    ...
    <div class="example-code-more" style="height: 49px;" v-if="ischange"><Button type="primary" :loading="aloading"  @click.native="submitUpdateAlbum" style="position: absolute;bottom: 10px;right: 10px;" >更新</Button></div>
    <div class="example-code-more" style="height: 49px;" v-else><Button type="primary" :loading="aloading" @click.native="submitAlbum" style="position: absolute;bottom: 10px;right: 10px;" >保存</Button></div>
          ...
          <ul  id="menu" v-for="(img,index) in BrowseImages" :key="index">
            <li>
              <div style="width: 64px;height: 64px;position: relative;padding: 2px;">
                <Icon @click="delBrowseImages(img.id,index)" type="md-close-circle" style="position: absolute; z-index: 1; right: -7px; top: -4px;cursor: pointer;" />
          ...
          <Row class="animate__animated animate__fadeIn animate__delay-1.5s">
            <Col flex="1" v-for="(item,index) in imglist" :key="index">
              <div class="imgdivstyle_exp divimgstyle-min_exp" >
                <span @click="noSelectImageClick(item.id)" class="formatTag_exp" v-show="selectImage.indexOf(item.id)>-1"><Icon type="md-done-all" size="50" style="margin-top: 50px;" /></span>
                <img @click="selectImageClick(index)" class="imgstyle-min_exp imgstyle"   v-lazy="item.imgurl+''" :src="item.imgurl+''"  :key="item.imgurl"   >
              </div>
            </Col>
          </Row>
